...
export async function generateMetadata(props: {
    ...
    const response: Array<ArticleInfo> =
        ((await query("SELECT * FROM article WHERE slug = ?", [
            props.params.slug,
...
export default async function ArticlePage(props: { params: { slug: string } }) {
    ...
    const response: ArticleInfoWithLike[] =
        ((await query(
            ...
            [ipAddress, props.params.slug]
        )) as ArticleInfoWithLike[]) || [];

    const articleInfo: ArticleInfoWithLike | null =
