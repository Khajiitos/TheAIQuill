	...
	return (
        <Link href={'article/' + props.articleInfo.slug}>
            <div className="bg-green-600 hover:bg-green-500 text-white container-lg rounded m-4 p-3 transition-colors">
                ...
                <p className="mt-2 text-gray-300 text-sm"><Image src="/img/clock_gray.svg" width={16} className="inline mr-2" alt="Reading time"></Image>{minutesOfReading} min read | {formattedDate}</p>
            </div>
        </Link>
	);
}